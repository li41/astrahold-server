...
type fencedEquipmentSink interface { EnqueueFencedEquipmentCommand(worldruntime.SessionOwnershipFence, uint32, protocol.ClientEquipmentCommand) error }
type fencedEquipmentInstanceSink interface { EnqueueFencedEquipmentInstanceCommand(worldruntime.SessionOwnershipFence, uint32, protocol.ClientEquipmentInstanceCommand) error }
type fencedEquipmentEnhancementSink interface { EnqueueFencedEnhanceEquipment(worldruntime.SessionOwnershipFence, uint32, protocol.ClientEnhanceEquipment) error }
type fencedPickupSink interface { EnqueueFencedPickupItem(worldruntime.SessionOwnershipFence, uint32, protocol.ClientPickupItem) error }
type fencedUseItemSink interface { EnqueueFencedUseItem(worldruntime.SessionOwnershipFence, uint32, protocol.ClientUseItem) error }
...
func (s ownedCommandSink) EnqueueEquipmentCommand(id session.ID, sequence uint32, command protocol.ClientEquipmentCommand) error {
	if !s.validSession(id) { return worldruntime.ErrCharacterOwnershipFenceInvalid }
	sink, ok := s.runtime.(fencedEquipmentSink); if !ok { return gateway.ErrUnsupportedClientMessage }
...
func (s ownedCommandSink) EnqueueEquipmentInstanceCommand(id session.ID, sequence uint32, command protocol.ClientEquipmentInstanceCommand) error {
	if !s.validSession(id) { return worldruntime.ErrCharacterOwnershipFenceInvalid }
	sink, ok := s.runtime.(fencedEquipmentInstanceSink); if !ok { return gateway.ErrUnsupportedClientMessage }
	return sink.EnqueueFencedEquipmentInstanceCommand(s.ownership, sequence, command)
}
func (s ownedCommandSink) EnqueueEnhanceEquipment(id session.ID, sequence uint32, command protocol.ClientEnhanceEquipment) error {
	if !s.validSession(id) { return worldruntime.ErrCharacterOwnershipFenceInvalid }
	sink, ok := s.runtime.(fencedEquipmentEnhancementSink); if !ok { return gateway.ErrUnsupportedClientMessage }
...
func (s ownedCommandSink) EnqueuePickupItem(id session.ID, sequence uint32, intent protocol.ClientPickupItem) error {
	if !s.validSession(id) { return worldruntime.ErrCharacterOwnershipFenceInvalid }
	sink, ok := s.runtime.(fencedPickupSink); if !ok { return gateway.ErrUnsupportedClientMessage }
...
func (s ownedCommandSink) EnqueueUseItem(id session.ID, sequence uint32, intent protocol.ClientUseItem) error {
	if !s.validSession(id) { return worldruntime.ErrCharacterOwnershipFenceInvalid }
	sink, ok := s.runtime.(fencedUseItemSink); if !ok { return gateway.ErrUnsupportedClientMessage }
...
func (s ownedCommandSink) EnqueueInteractNPC(id session.ID, sequence uint32, intent protocol.ClientInteractNPC) error {
	if !s.validSession(id) { return worldruntime.ErrCharacterOwnershipFenceInvalid }
	sink, ok := s.runtime.(fencedNPCSink); if !ok { return gateway.ErrUnsupportedClientMessage }
...
func (s ownedCommandSink) EnqueueShopCommand(id session.ID, sequence uint32, intent protocol.ClientShopCommand) error {
	if !s.validSession(id) { return worldruntime.ErrCharacterOwnershipFenceInvalid }
	sink, ok := s.runtime.(fencedShopSink); if !ok { return gateway.ErrUnsupportedClientMessage }
	return sink.EnqueueFencedShopCommand(s.ownership, sequence, intent)
}
func (s ownedCommandSink) EnqueueWarehouseCommand(id session.ID, sequence uint32, intent protocol.ClientWarehouseCommand) error {
	if !s.validSession(id) { return worldruntime.ErrCharacterOwnershipFenceInvalid }
...
var (
	...
	_ gateway.EquipmentEnhancementCommandSink = ownedCommandSink{}
