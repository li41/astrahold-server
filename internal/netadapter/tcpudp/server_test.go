...
func newFakeRuntime() *fakeRuntime {
	return &fakeRuntime{
		...
		actions: make(chan actionCall, 4),
	}
}
...
func (f *fakeRuntime) EnqueueUseAction(id session.ID, seq uint32, action protocol.ClientUseAction) error {
	f.actions <- actionCall{id: id, seq: seq, action: action}
	return nil
}

func TestDefaultCharacterIdentityFactoryIsEphemeral(t *testing.T) {
	cfg := DefaultConfig()
	server := NewServer(cfg, newFakeRuntime(), gamev1.Codec{})
	binding, err := server.config.CharacterIdentityFactory(1, 1)
	if err != nil {
		t.Fatal(err)
	}
	if !binding.Valid() || binding.Assurance != characteridentity.AssuranceEphemeral {
...
func TestTCPUDPHandshakeAndRouting(t *testing.T) {
	...
	trusted, err := characteridentity.NewTrusted("character:tcp-test")
	if err != nil {
		t.Fatal(err)
	}
	...
	if welcome.World != cfg.WorldIdentity {
		t.Fatalf("world identity mismatch: got=%+v want=%+v", welcome.World, cfg.WorldIdentity)
	}
	...
	if uint64(join.Session.ID) != welcome.SessionID || join.Entity.ID != welcome.EntityID {
		t.Fatal("join/welcome mismatch")
	}
	if join.Session.CharacterIdentity != trusted {
		t.Fatalf("character identity=%#v want=%#v", join.Session.CharacterIdentity, trusted)
	}

	if err := join.Session.Connection().TrySend(protocol.Envelope{Delivery: protocol.DeliveryReliableOrdered, Sequence: 1, Message: protocol.EntityDespawn{EntityID: 999}}); err != nil {
		t.Fatal(err)
	}
	...
	case call := <-runtime.actions:
		if uint64(call.id) != welcome.SessionID || call.seq != 7 || call.action != actionMessage {
			t.Fatalf("action route mismatch: %#v", call)
		}
	case <-time.After(time.Second):
		t.Fatal("action not routed")
	}
	...
	if _, ok := got.Message.(protocol.PositionCorrection); !ok {
		t.Fatalf("unexpected realtime message: %#v", got.Message)
	}
