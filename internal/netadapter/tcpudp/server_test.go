...
func TestTCPUDPHandshakeAndRouting(t *testing.T) {
	...
	case move := <-runtime.moves:
		if uint64(move.id) != welcome.SessionID || move.seq != 3 || move.input.DirectionX != 1 { t.Fatalf("move mismatch: %#v", move) }
	case <-time.After(time.Second): t.Fatal("move not routed")
	}
	...
	case id := <-runtime.leaves:
		if uint64(id) != welcome.SessionID { t.Fatal("leave mismatch") }
	case <-time.After(time.Second): t.Fatal("leave not enqueued")
	}

	cancel(); _ = server.Close()
	select {
	case err := <-serveDone:
		if err != nil { t.Fatalf("serve error: %v", err) }
	case <-time.After(time.Second): t.Fatal("serve did not stop")
	}
}
