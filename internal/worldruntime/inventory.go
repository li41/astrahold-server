...
func durableInventoryState(inv *inventory.Inventory) (characterstate.InventoryState, error) {
	...
	if err := validateInventoryHandCombination(inv); err != nil {
	...
	for _, item := range inv.EquippedArchetypeSnapshot() {
		kind, ok := expectedEquipmentKind(item.Slot)
		if !ok {
			return characterstate.InventoryState{}, ErrEquipmentItemNotAllowed
		}
		catalogSlot, ok := catalogEquipmentSlot(item.Slot)
		if !ok || !legacyEquipmentArchetypeAllowed(item.ArchetypeID, kind, catalogSlot) {
			return characterstate.InventoryState{}, ErrEquipmentItemNotAllowed
		}
	...
	for _, equipped := range inv.EquippedInstanceSnapshot() {
		kind, ok := expectedEquipmentKind(equipped.Slot)
		if !ok {
			return characterstate.InventoryState{}, ErrEquipmentItemNotAllowed
		}
		catalogSlot, ok := catalogEquipmentSlot(equipped.Slot)
		if !ok {
			return characterstate.InventoryState{}, ErrEquipmentItemNotAllowed
		}
		if err := validateDurableEquipmentInstance(equipped.Item, kind, catalogSlot); err != nil {
			return characterstate.InventoryState{}, err
		}
		data, err := iteminstance.CanonicalShapeJSON(equipped.Item)
		if err != nil {
			return characterstate.InventoryState{}, err
		}
...
func restoreCharacterInventory(maxStacks int, state characterstate.InventoryState) (*inventory.Inventory, error) {
	...
	if err != nil {
		return nil, characterstate.ErrInvalidSnapshot
	}
	state = canonical
	if err := validateInventoryStateHandCombination(state); err != nil {
		return nil, err
	}
	...
	for _, item := range equipment {
		slot := inventory.EquipmentSlot(item.Slot)
		kind, ok := expectedEquipmentKind(slot)
		if !ok {
			return nil, ErrEquipmentItemNotAllowed
		}
		catalogSlot, ok := catalogEquipmentSlot(slot)
		if !ok || !legacyEquipmentArchetypeAllowed(item.ItemArchetypeID, kind, catalogSlot) {
			return nil, ErrEquipmentItemNotAllowed
		}
		if err := inv.Add(item.ItemArchetypeID, 1); err != nil {
			return nil, err
		}
		if err := inv.Equip(slot, item.ItemArchetypeID); err != nil {
			return nil, err
		}
	}
	for _, item := range equippedInstances {
		...
		instance, err := iteminstance.DecodeCanonicalShapeJSON([]byte(item.ItemInstanceJSON))
		if err != nil {
			return nil, err
		}
		if err := validateDurableEquipmentInstance(instance, kind, catalogSlot); err != nil {
			return nil, err
		}
		if err := inv.AddInstance(instance); err != nil {
			return nil, err
		}
		if err := inv.EquipInstance(slot, instance.ID); err != nil {
			return nil, err
		}
	}
	for _, instance := range instances {
...
func buildEquipmentArchetypeSlots(inv *inventory.Inventory) []protocol.EquipmentSlotState {
	if inv == nil {
		return nil
	}
	slots := make([]protocol.EquipmentSlotState, 0, 7)
	for _, inventorySlot := range inventory.EquipmentSlots() {
		itemArchetypeID := inv.Equipped(inventorySlot)
		if itemArchetypeID == "" {
			continue
		}
		slot, ok := protocolEquipmentSlot(inventorySlot)
		if !ok {
			continue
		}
		slots = append(slots, protocol.EquipmentSlotState{Slot: slot, ItemArchetypeID: itemArchetypeID})
	}
	return slots
}
