...
func validateStarterInventory(maxStacks int, stacks []inventory.Stack) error {
	...
	for _, stack := range stacks {
		if err := inv.Add(stack.ArchetypeID, stack.Quantity); err != nil { return err }
	}
	return nil
}
...
func durableInventoryState(inv *inventory.Inventory) (characterstate.InventoryState, error) {
	...
	for _, stack := range stacks {
		durable = append(durable, characterstate.InventoryStack{ItemArchetypeID: stack.ArchetypeID, Quantity: stack.Quantity})
	}
...
func restoreCharacterInventory(maxStacks int, state characterstate.InventoryState) (*inventory.Inventory, error) {
	...
	inv := newCharacterInventory(maxStacks)
	if state.MainHand != "" {
		if state.MainHand != trainingBladeArchetypeID { return nil, ErrEquipmentItemNotAllowed }
...
func (r *Runtime) ensureSessionInventory(s *session.Session) {
	...
	if _, ok := r.inventories[identity]; !ok {
		...
		for _, stack := range r.config.StarterInventory {
			if err := inv.Add(stack.ArchetypeID, stack.Quantity); err != nil { panic(err) }
		}
...
func (r *Runtime) replicatePendingInventories(tick uint64, report *StepReport) {
	...
	for _, s := range r.sessions.List() {
		...
		report.Metrics.OutboundMessages++
		if err:=s.Connection().TrySend(equipmentEnvelope);err!=nil{if !errors.Is(err,session.ErrBackpressure){report.DeliveryErrors=append(report.DeliveryErrors,DeliveryError{SessionID:s.ID,Delivery:equipmentEnvelope.Delivery,MessageType:equipmentMessage.Type(),Err:err})};continue}
