...
type CommandError struct { Command string; SessionID session.ID; Err error }
type ActionRejection struct { Action string; SessionID session.ID; Err error }
type DeliveryError struct { SessionID session.ID; Delivery protocol.Delivery; MessageType protocol.MessageType; Err error }
...
func New(w *simulation.World, config Config, options ...Option) *Runtime {
	...
	if config.CharacterMaxHP == 0 { config.CharacterMaxHP = 1000 }
