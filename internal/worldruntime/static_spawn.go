...
// SpawnEntityRequest describes a server-owned non-session combat entity. BodySize is authored
// gameplay classification used by weapon damage; an empty value remains compatibility data and
// resolves to the small-damage table until that archetype is explicitly classified.
...
func validateSpawnEntityRequest(request SpawnEntityRequest) error {
	if request.Entity.ID == 0 || !combatantKind(request.Entity.Kind) || request.Entity.Kind == world.EntityPlayer { return ErrInvalidSpawnEntityRequest }
	if request.Entity.ArchetypeID == "" || request.HP == 0 || request.MaxHP == 0 || request.HP > request.MaxHP { return ErrInvalidSpawnEntityRequest }
	for _, value := range []float32{request.Speed, request.Radius, request.MaxStepHeight} {
		if math.IsNaN(float64(value)) || math.IsInf(float64(value), 0) || value < 0 { return ErrInvalidSpawnEntityRequest }
	}
	if request.Radius == 0 { return ErrInvalidSpawnEntityRequest }
	switch request.BodySize {
...
func (r *Runtime) EnqueueSpawnEntity(request SpawnEntityRequest) error {
	if err := validateSpawnEntityRequest(request); err != nil { return err }
