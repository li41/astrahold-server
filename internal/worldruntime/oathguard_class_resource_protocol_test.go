...
func TestOathguardInitialClassCommitPublishesClassResourceBeforeResult(t *testing.T) {
	rt, outbox, fence := joinInitialClassAssignmentCharacter(t)
	conn := reliableConnectionForFence(t, rt, fence)
	drainReliable(conn)

	if err := rt.EnqueueFencedInitialClassSelection(fence, 31, protocol.ClientInitialClassSelection{ClassID: string(classid.Oathguard)}); err != nil {
		t.Fatal(err)
	}
	...
	if !ok || classState.ClassID != string(classid.Oathguard) {
		t.Fatalf("first message=%#v", messages[0])
	}
	resourceState, ok := messages[1].(protocol.CharacterClassResourceState)
	if !ok || resourceState.EntityID != fence.EntityID || resourceState.ResourceID != "resolve" || resourceState.Current != 0 || resourceState.Max != 100 {
		t.Fatalf("second message=%#v", messages[1])
	}
	result, ok := messages[2].(protocol.InitialClassSelectionResult)
	if !ok || result.ClientActionSequence != 31 || result.ClassID != string(classid.Oathguard) || result.Outcome != protocol.InitialClassSelectionCommitted {
