...
func (r *Runtime) dispatchPreparedAction(name string, command useActionCommand, entityID session.ID, prepared combat.PreparedAction, tick uint64, delta time.Duration, report *StepReport) {
	actorSession, _ := r.sessions.Get(command.sessionID)
