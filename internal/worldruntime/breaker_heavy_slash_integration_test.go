...
func TestBreakerHeavySlashHitGainsMomentumAndCooldownDoesNot(t *testing.T) {
	rt, s, conn, monsterID := newBreakerHeavySlashRuntime(t)
	...
	intent := protocol.ClientUseAction{
		ActionID:   classaction.BreakerHeavySlash,
	...
	if !ok || monster.HP != 165 {
		t.Fatalf("monster=%+v ok=%v, want hp=165", monster, ok)
	}
	actor, ok := rt.characters.State(s.EntityID)
	if !ok || actor.ClassResourceID != classresource.Momentum || actor.ClassResource != 10 || actor.MaxClassResource != 100 {
	...
	for _, envelope := range drainReliable(conn) {
		if state, ok := envelope.Message.(protocol.CharacterClassResourceState); ok {
			foundResource = true
			if state.EntityID != s.EntityID || state.ResourceID != "momentum" || state.Current != 10 || state.Max != 100 {
	...
	if actor.ClassResource != 10 {
		t.Fatalf("cooldown rejection changed momentum=%d", actor.ClassResource)
	}
...
func TestBreakerHeavySlashRejectsWrongClassBeforeDamage(t *testing.T) {
	rt, s, conn, monsterID := newBreakerHeavySlashRuntime(t)
	...
	found := false
	for _, envelope := range drainReliable(conn) {
		if rejected, ok := envelope.Message.(protocol.ActionRejected); ok && rejected.ActionID == classaction.BreakerHeavySlash {
			found = true
...
func newBreakerHeavySlashRuntime(t *testing.T) (*Runtime, *session.Session, *session.QueueConnection, world.EntityID) {
	t.Helper()
	definition := gameplayworld.Definition{
		SchemaVersion: gameplayworld.SchemaVersion,
		WorldID:       "breaker-heavy-slash-test",
		Revision:      "r1",
		Units:         "meters",
	...
	combatService, err := combat.NewService([]combat.ActionDefinition{{
		ID:              classaction.BreakerHeavySlash,
