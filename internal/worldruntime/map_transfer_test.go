...
func TestMap0ExitWritesMap1SnapshotThenRemovesSource(t *testing.T) {
	...
	if err := rt.EnqueueMapExit(1, MapExitRequest{DestinationWorld: map1TestWorld, DestinationTransform: destination}); err != nil {
		t.Fatal(err)
	}
	report := rt.Step(2, 50*time.Millisecond)
	if len(report.CommandErrors) != 0 {
		t.Fatalf("exit errors=%#v", report.CommandErrors)
	}
	...
	got := pending[0]
	if got.Identity != identity || got.Snapshot.World != map1TestWorld || got.Snapshot.Position != destination.Position || got.Snapshot.Yaw != destination.Yaw {
		t.Fatalf("transfer snapshot=%#v", got.Snapshot)
	}
	stacks, err := got.Snapshot.Inventory.Stacks()
	if err != nil {
		t.Fatal(err)
	}
	if quantityOfStack(stacks, "item_training_blade") != 1 {
		t.Fatalf("training blade lost during transfer stacks=%#v", stacks)
	}
	if _, ok := rt.sessions.Get(1); ok {
		t.Fatal("source session remained after successful map exit")
	}
	if _, ok := rt.world.Entity(1); ok {
		t.Fatal("source entity remained after successful map exit")
	}
}

func TestMap0ExitServerPolicyBlocksRestrictedItemWithoutRequestRestriction(t *testing.T) {
	outbox, _ := characterstate.NewOutbox(4)
	rt, identity := makeMap0TransferRuntime(t, outbox)
	before := rt.inventories[identity.ID].Snapshot()
	request := MapExitRequest{DestinationWorld: map1TestWorld}
	report := StepReport{}
	rt.applyMapExitWithPolicy("map_exit", leaveCommand{id: 1, mapExit: &request}, gameplayworld.MapExitPolicy{RestrictedItemArchetypeIDs: []string{"item_training_blade"}}, &report)
	if len(report.CommandErrors) != 1 || !errors.Is(report.CommandErrors[0].Err, ErrMapExitRestrictedItem) {
		t.Fatalf("errors=%#v", report.CommandErrors)
	}
	if outbox.Depth() != 0 {
		t.Fatalf("blocked exit enqueued save depth=%d", outbox.Depth())
	}
	if _, ok := rt.sessions.Get(1); !ok {
		t.Fatal("blocked exit removed source session")
	}
	if _, ok := rt.world.Entity(1); !ok {
		t.Fatal("blocked exit removed source entity")
	}
...
func TestMap0ExitServerPolicyBlocksRestrictedEquippedItem(t *testing.T) {
	outbox, _ := characterstate.NewOutbox(4)
	rt, identity := makeMap0TransferRuntime(t, outbox)
	...
	if inv.MainHand() != "item_training_blade" || outbox.Depth() != 0 {
		t.Fatalf("blocked equipped item mutated main_hand=%q depth=%d", inv.MainHand(), outbox.Depth())
	}
}
...
func TestMap0ExitOutboxFailureRollsBackSessionRegistry(t *testing.T) {
	...
	if err := rt.EnqueueMapExit(1, MapExitRequest{DestinationWorld: map1TestWorld}); err != nil {
		t.Fatal(err)
	}
	report := rt.Step(2, 50*time.Millisecond)
...
func TestMapExitRequiresIsolatedSource(t *testing.T) {
	...
	if err := rt.EnqueueMapExit(1, MapExitRequest{DestinationWorld: map0TestWorld}); err != nil {
		t.Fatal(err)
	}
	report := rt.Step(2, 50*time.Millisecond)
	if len(report.CommandErrors) != 1 || !errors.Is(report.CommandErrors[0].Err, ErrMapExitSourceNotIsolated) {
		t.Fatalf("errors=%#v", report.CommandErrors)
	}
	if _, ok := rt.sessions.Get(1); !ok {
		t.Fatal("non-isolated source was mutated")
	}
}
...
func TestSuccessfulMap0ExitSnapshotRestoresIntoMap1Runtime(t *testing.T) {
	...
	if err := source.EnqueueMapExit(1, MapExitRequest{DestinationWorld: map1TestWorld, DestinationTransform: destination}); err != nil {
		t.Fatal(err)
	}
	if report := source.Step(2, 50*time.Millisecond); len(report.CommandErrors) != 0 {
		t.Fatalf("source exit errors=%#v", report.CommandErrors)
	}
	pending := outbox.Pending(1)
	if len(pending) != 1 {
		t.Fatalf("pending=%#v", pending)
	...
	if restore.MapID != "map1" {
		t.Fatalf("restore map=%q", restore.MapID)
	}
	...
	sess, err := session.NewWithCharacterIdentity(2, 1, identity, 64, session.NewQueueConnection(32, 32))
	if err != nil {
		t.Fatal(err)
	}
	if err := destinationRuntime.EnqueueJoin(JoinRequest{Session: sess, Entity: world.EntityState{ID: 1, Kind: world.EntityPlayer}, Speed: 6, Radius: 0.35, MaxStepHeight: 0.5, Restore: &restore}); err != nil {
		t.Fatal(err)
	}
	report := destinationRuntime.Step(1, 50*time.Millisecond)
	if len(report.CommandErrors) != 0 {
		t.Fatalf("restore errors=%#v", report.CommandErrors)
	}
	entity, ok := destinationRuntime.world.Entity(1)
	if !ok || entity.Transform != destination {
		t.Fatalf("restored entity=%#v ok=%v", entity, ok)
	}
}
...
func TestRestrictedItemScanIncludesInstancesAndEquippedInstances(t *testing.T) {
	...
	if err != nil || got != "item_restricted_instance" {
		t.Fatalf("unequipped instance got=%q err=%v", got, err)
	}
	...
	got, err = firstRestrictedItemArchetype(inv, []string{"item_restricted_instance"})
	if err != nil || got != "item_restricted_instance" {
		t.Fatalf("equipped instance got=%q err=%v", got, err)
	}
}
...
func makeMapTransferRuntime(t *testing.T, outbox *characterstate.Outbox, worldRef characterstate.WorldRef) (*Runtime, characteridentity.Binding) {
	...
	identity, err := characteridentity.NewTrusted("character:map-transfer")
	if err != nil {
		t.Fatal(err)
	}
	sess, err := session.NewWithCharacterIdentity(1, 1, identity, 64, session.NewQueueConnection(32, 32))
	if err != nil {
		t.Fatal(err)
	}
