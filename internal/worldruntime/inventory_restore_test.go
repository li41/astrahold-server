...
func TestJoinRestoresInitializedInventoryAndMainHandBeforeFirstSnapshot(t *testing.T) {
	...
	restore := CharacterRestore{
		...
		HP: 700, MaxHP: 1000, MP: 60, MaxMP: 100, PrimaryStats: characterstats.DefaultPrimary(),
	...
	if inv.MainHand() != "item_training_blade" || inv.Quantity("item_minor_healing_potion") != 2 || inv.Quantity("item_minor_mana_potion") != 3 { t.Fatalf("mainhand=%q inventory=%#v", inv.MainHand(), inv.Snapshot()) }
...
func TestJoinInitializedEmptyInventoryDoesNotBootstrapStarterItems(t *testing.T) {
	...
	if err != nil { t.Fatal(err) }
	restore := CharacterRestore{SchemaVersion:characterstate.InventorySchemaVersion,CharacterID:identity.ID,Revision:1,World:characterRestoreWorld,HP:1000,MaxHP:1000,MP:100,MaxMP:100,PrimaryStats:characterstats.DefaultPrimary(),Transform:world.Transform{Position:world.Position{Layer:4}},Inventory:empty}
	if err := rt.EnqueueJoin(JoinRequest{Session:sess,Entity:world.EntityState{ID:1,Kind:world.EntityPlayer},Speed:6,Radius:0.35,MaxStepHeight:0.5,Restore:&restore});err!=nil{t.Fatal(err)}
...
func TestJoinRejectsIllegalDurableMainHandBeforeWorldMutation(t *testing.T) {
	...
	if err != nil { t.Fatal(err) }
	restore := CharacterRestore{SchemaVersion:characterstate.InventorySchemaVersion,CharacterID:identity.ID,Revision:1,World:characterRestoreWorld,HP:1000,MaxHP:1000,MP:100,MaxMP:100,PrimaryStats:characterstats.DefaultPrimary(),Transform:world.Transform{Position:world.Position{Layer:4}},Inventory:persisted}
	if err := rt.EnqueueJoin(JoinRequest{Session:sess,Entity:world.EntityState{ID:1,Kind:world.EntityPlayer},Speed:6,Radius:0.35,MaxStepHeight:0.5,Restore:&restore});err!=nil{t.Fatal(err)}
	report:=rt.Step(1,50*time.Millisecond)
