...
func TestTrustedLeaveEnqueuesAuthoritativeCharacterState(t *testing.T) {
	...
	if len(report.CommandErrors) != 0 || report.Metrics.CharacterStateSaveIntentsEnqueued != 1 || report.Metrics.CharacterStateSaveIntentFailures != 0 { t.Fatalf("leave=%#v", report) }
	pending := outbox.Pending(1)
	if len(pending) != 1 { t.Fatalf("pending=%#v", pending) }
	intent := pending[0]
	wantPosition := world.Position{X: 7, Y: 0, Z: -3, Layer: 4}
	if intent.Identity != identity || intent.Snapshot.World != characterStateTestWorld || intent.Snapshot.HP != 875 || intent.Snapshot.MaxHP != 1000 || intent.Snapshot.MP != 100 || intent.Snapshot.MaxMP != 100 || intent.Snapshot.Defeated || intent.Snapshot.Position != wantPosition || intent.Snapshot.Yaw != 1.25 || intent.Snapshot.PrimaryStats != characterstats.DefaultPrimary() { t.Fatalf("intent=%#v", intent) }
	if _, ok := rt.world.Entity(1); ok { t.Fatal("leave did not remove world entity") }
	if _, ok := rt.characters.State(1); ok { t.Fatal("leave did not remove character state") }
}
...
func TestEphemeralLeaveDoesNotEnterDurableSaveOutbox(t *testing.T) {
	...
	if err := rt.EnqueueLeave(1); err != nil { t.Fatal(err) }
	report := rt.Step(2, 50*time.Millisecond)
	if len(report.CommandErrors) != 0 || report.Metrics.CharacterStateSaveIntentsEnqueued != 0 || report.Metrics.CharacterStateSaveIntentFailures != 0 || outbox.Depth() != 0 { t.Fatalf("report=%#v depth=%d", report, outbox.Depth()) }
}
...
func TestCharacterStateOutboxFailureDoesNotRollbackLeave(t *testing.T) {
	...
	blockerSnapshot := characterstate.Snapshot{
		...
		Position: world.Position{Layer: 0}, PrimaryStats: characterstats.DefaultPrimary(),
	}
	...
	if report.Metrics.CharacterStateSaveIntentsEnqueued != 0 || report.Metrics.CharacterStateSaveIntentFailures != 1 { t.Fatalf("metrics=%#v", report.Metrics) }
	if _, ok := rt.sessions.Get(1); ok { t.Fatal("save failure kept session registered") }
	if _, ok := rt.world.Entity(1); ok { t.Fatal("save failure kept world entity alive") }
...
func makeCharacterStateRuntime(t *testing.T, outbox *characterstate.Outbox) *Runtime {
	...
	entity := world.EntityState{ID: 1, Kind: world.EntityPlayer, Transform: world.Transform{Position: world.Position{X: 7, Y: 2, Z: -3, Layer: 4}, Yaw: 1.25}}
