...
func TestCharacterSkillStateRestoresSavesCurrentRuntimeAndClearsOnLeave(t *testing.T) {
	...
	restore := CharacterRestore{
		...
		CombatLoadout: initialLoadout, LearnedSkills: initialLearned,
	}
	...
	if gotLearned != initialLearned || rt.characterSkills.loadout.Slots(1) != initialLoadout { t.Fatalf("restored learned=%v loadout=%v", gotLearned.IDs(), rt.characterSkills.loadout.Combat(1)) }
	...
	if err := rt.characterSkills.learned.SetLearned(1, currentLearned.IDs()); err != nil { t.Fatal(err) }
	if err := rt.characterSkills.loadout.SetCombat(1, currentLoadout.IDs(), func(id skillcatalog.ID) bool { return rt.characterSkills.learned.Contains(1, id) }); err != nil { t.Fatal(err) }

	if err := rt.EnqueueLeave(1); err != nil { t.Fatal(err) }
	...
	if pending[0].Snapshot.LearnedSkills != currentLearned || pending[0].Snapshot.CombatLoadout != currentLoadout { t.Fatalf("saved learned=%v loadout=%v", pending[0].Snapshot.LearnedSkills.IDs(), pending[0].Snapshot.CombatLoadout.IDs()) }
	if rt.characterSkills.learned.Count(1) != 0 || rt.characterSkills.loadout.Slots(1) != (skillloadout.Slots{}) { t.Fatalf("teardown left learned=%v loadout=%v", rt.characterSkills.learned.Learned(1), rt.characterSkills.loadout.Combat(1)) }
}
...
func TestJoinLateFailureRollsBackRestoredCharacterSkillState(t *testing.T) {
	...
	if err != nil { t.Fatal(err) }
	if err := rt.world.Spawn(world.EntityState{ID: 99, Kind: world.EntityPlayer, Transform: world.Transform{Position: world.Position{Layer: 4}}}, 6, 0.35, 0.5); err != nil { t.Fatal(err) }
	if err := rt.EnqueueRegister(existing); err != nil { t.Fatal(err) }
	if report := rt.Step(1, 50*time.Millisecond); len(report.CommandErrors) != 0 { t.Fatalf("existing register errors=%#v", report.CommandErrors) }
	...
	if err := rt.EnqueueJoin(JoinRequest{Session: incoming, Entity: world.EntityState{ID: 1, Kind: world.EntityPlayer}, Speed: 6, Radius: 0.35, MaxStepHeight: 0.5, Restore: &restore}); err != nil { t.Fatal(err) }
	report := rt.Step(2, 50*time.Millisecond)
	if len(report.CommandErrors) != 1 || !errors.Is(report.CommandErrors[0].Err, session.ErrSessionExists) { t.Fatalf("errors=%#v", report.CommandErrors) }
	if _, ok := rt.world.Entity(1); ok { t.Fatal("late join failure left world entity") }
	if _, ok := rt.characters.State(1); ok { t.Fatal("late join failure left character state") }
	if rt.characterSkills.learned.Count(1) != 0 || rt.characterSkills.loadout.Slots(1) != (skillloadout.Slots{}) { t.Fatalf("late join failure left learned=%v loadout=%v", rt.characterSkills.learned.Learned(1), rt.characterSkills.loadout.Combat(1)) }
	if got, ok := rt.sessions.Get(1); !ok || got != existing { t.Fatalf("existing session changed got=%#v ok=%v", got, ok) }
}
...
func TestJoinRejectsUnlearnedCombatLoadoutBeforeWorldMutation(t *testing.T) {
	...
	if len(report.CommandErrors) != 1 || !errors.Is(report.CommandErrors[0].Err, ErrCharacterRestoreInvalid) { t.Fatalf("errors=%#v", report.CommandErrors) }
	assertRestoreJoinDidNotPartiallySpawn(t, rt)
	if rt.characterSkills.learned.Count(1) != 0 || rt.characterSkills.loadout.Slots(1) != (skillloadout.Slots{}) { t.Fatalf("invalid restore left learned=%v loadout=%v", rt.characterSkills.learned.Learned(1), rt.characterSkills.loadout.Combat(1)) }
}
...
func TestValidateCharacterRestoreAcceptsV7InferredLearnedSkills(t *testing.T) {
	...
	if err := ValidateCharacterRestore(identity, restore, characterRestoreWorld); err != nil { t.Fatalf("v7 inferred restore err=%v", err) }
}
