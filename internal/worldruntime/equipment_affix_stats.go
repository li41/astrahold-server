...
func (r *Runtime) equippedInstanceModifiers(entityID world.EntityID) (equipmentstats.Modifiers, error) {
	...
	if !ok || !s.CharacterIdentity.Valid() { return equipmentstats.Modifiers{}, nil }
	inv := r.inventories[s.CharacterIdentity.ID]
	if inv == nil { return equipmentstats.Modifiers{}, nil }
	...
	enhancement := equipmentstats.Modifiers{}
	appendStatic := func(itemArchetypeID string) {
		...
		if found { staticModifiers = append(staticModifiers, modifiers...) }
	}
	...
	for _, equipped := range inv.EquippedInstanceSnapshot() {
		...
		if definition, found := defaultEquipmentCatalog.Resolve(equipped.Item.ItemArchetypeID); found {
			switch definition.Kind {
			case equipmentcatalog.KindWeapon:
				enhancement.PhysicalDamage += uint32(equipped.Item.EnhancementLevel)
			case equipmentcatalog.KindArmor, equipmentcatalog.KindShield:
				enhancement.PhysicalDefense += uint32(equipped.Item.EnhancementLevel)
			}
		}
	}
	setModifiers, err := defaultEquipmentCatalog.SetStaticModifiersForEquipped(itemArchetypeIDs)
	if err != nil { return equipmentstats.Modifiers{}, err }
	...
	if err != nil { return equipmentstats.Modifiers{}, err }
	return equipmentstats.Merge(fixed, rolled, enhancement)
}
