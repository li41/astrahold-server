...
func validateInventoryStateHandCombination(state characterstate.InventoryState) error {
	...
	for _, item := range equipment {
		switch item.Slot {
		case "main_hand":
			mainHand = item.ItemArchetypeID
		case "off_hand":
			offHand = item.ItemArchetypeID
		}
	}
	instances, err := state.EquipmentInstances()
	if err != nil {
		return err
	}
...
// applyEquipArchetype keeps the protocol slot at the worldruntime boundary for compatibility with
// command-path tests, then converts immediately into the inventory-owned slot type.
func applyEquipArchetype(inv *inventory.Inventory, protocolSlot protocol.EquipmentSlot, itemArchetypeID string) error {
	slot, ok := inventoryEquipmentSlot(protocolSlot)
	if !ok {
		return ErrEquipmentItemNotAllowed
	}
	if !lowTierEquipmentAllowed(slot, itemArchetypeID) {
		return ErrEquipmentItemNotAllowed
	}
