...
func TestRangerHuntingShotHitsAtRangeAndGainsHuntMomentum(t *testing.T) {
	rt, s, conn, monsterID := newRangerHuntingShotRuntime(t, 10)
	...
	intent := protocol.ClientUseAction{
		ActionID:   classaction.RangerHuntingShot,
	...
	if !ok || monster.HP != 150 {
		t.Fatalf("monster=%+v ok=%v, want hp=150", monster, ok)
	}
	actor, ok := rt.characters.State(s.EntityID)
	if !ok || actor.ClassResourceID != classresource.HuntMomentum || actor.ClassResource != 8 || actor.MaxClassResource != 100 {
	...
	for _, envelope := range drainReliable(conn) {
		if state, ok := envelope.Message.(protocol.CharacterClassResourceState); ok {
			foundResource = true
			if state.EntityID != s.EntityID || state.ResourceID != "hunt_momentum" || state.Current != 8 || state.Max != 100 {
				t.Fatalf("resource state=%#v", state)
			}
		}
	}
	if !foundResource {
		t.Fatal("missing authoritative hunt momentum state after hit")
	}
	...
	if actor.ClassResource != 8 {
		t.Fatalf("cooldown rejection changed hunt momentum=%d", actor.ClassResource)
	}
...
func TestRangerHuntingShotOutOfRangeDoesNotDamageOrGainResource(t *testing.T) {
	rt, s, conn, monsterID := newRangerHuntingShotRuntime(t, 14)
	if _, err := rt.characters.AssignInitialClass(s.EntityID, classid.Ranger); err != nil {
		t.Fatal(err)
	}
	drainReliable(conn)
	...
	if actor.ClassResource != 0 {
		t.Fatalf("out-of-range request changed hunt momentum=%d", actor.ClassResource)
	}
}

func TestRangerHuntingShotRejectsWrongClassBeforeDamage(t *testing.T) {
	rt, s, conn, monsterID := newRangerHuntingShotRuntime(t, 10)
	...
	found := false
	for _, envelope := range drainReliable(conn) {
		if rejected, ok := envelope.Message.(protocol.ActionRejected); ok && rejected.ActionID == classaction.RangerHuntingShot {
			found = true
...
func newRangerHuntingShotRuntime(t *testing.T, monsterX float32) (*Runtime, *session.Session, *session.QueueConnection, world.EntityID) {
	...
	definition := gameplayworld.Definition{
		...
		WorldID:       "ranger-hunting-shot-test",
		Revision:      "r1",
		Units:         "meters",
	...
	combatService, err := combat.NewService([]combat.ActionDefinition{{
		ID:              classaction.RangerHuntingShot,
