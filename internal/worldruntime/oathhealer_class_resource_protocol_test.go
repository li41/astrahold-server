...
func TestOathhealerInitialClassCommitPublishesOathSealBeforeResult(t *testing.T) {
	rt, outbox, fence := joinInitialClassAssignmentCharacter(t)
	conn := reliableConnectionForFence(t, rt, fence)
	drainReliable(conn)

	if err := rt.EnqueueFencedInitialClassSelection(fence, 35, protocol.ClientInitialClassSelection{ClassID: string(classid.Oathhealer)}); err != nil {
		t.Fatal(err)
	}
	...
	if state, _ := rt.characters.State(fence.EntityID); state.ClassID != "" || state.ClassResourceID != "" || state.ClassResourceProgress != 0 {
	...
	if !ok || state.ClassID != classid.Oathhealer || state.ClassResourceID != "oath_seal" || state.ClassResource != 0 || state.MaxClassResource != 3 || state.ClassResourceProgress != 0 {
	...
	if !ok || classState.ClassID != string(classid.Oathhealer) {
		t.Fatalf("first message=%#v", messages[0])
	}
	resourceState, ok := messages[1].(protocol.CharacterClassResourceState)
	if !ok || resourceState.EntityID != fence.EntityID || resourceState.ResourceID != "oath_seal" || resourceState.Current != 0 || resourceState.Max != 3 {
		t.Fatalf("second message=%#v", messages[1])
	}
	result, ok := messages[2].(protocol.InitialClassSelectionResult)
	if !ok || result.ClientActionSequence != 35 || result.ClassID != string(classid.Oathhealer) || result.Outcome != protocol.InitialClassSelectionCommitted {
