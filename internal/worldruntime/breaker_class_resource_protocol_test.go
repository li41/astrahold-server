...
func TestBreakerInitialClassCommitPublishesMomentumBeforeResult(t *testing.T) {
	rt, outbox, fence := joinInitialClassAssignmentCharacter(t)
	conn := reliableConnectionForFence(t, rt, fence)
	drainReliable(conn)

	if err := rt.EnqueueFencedInitialClassSelection(fence, 32, protocol.ClientInitialClassSelection{ClassID: string(classid.Breaker)}); err != nil {
		t.Fatal(err)
	}
	...
	if !ok || state.ClassID != classid.Breaker || state.ClassResourceID != "momentum" || state.ClassResource != 0 || state.MaxClassResource != 100 {
	...
	if !ok || classState.ClassID != string(classid.Breaker) {
		t.Fatalf("first message=%#v", messages[0])
	}
	resourceState, ok := messages[1].(protocol.CharacterClassResourceState)
	if !ok || resourceState.EntityID != fence.EntityID || resourceState.ResourceID != "momentum" || resourceState.Current != 0 || resourceState.Max != 100 {
		t.Fatalf("second message=%#v", messages[1])
	}
	result, ok := messages[2].(protocol.InitialClassSelectionResult)
	if !ok || result.ClientActionSequence != 32 || result.ClassID != string(classid.Breaker) || result.Outcome != protocol.InitialClassSelectionCommitted {
