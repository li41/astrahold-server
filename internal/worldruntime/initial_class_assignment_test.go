...
func TestInitialClassAssignmentCommitsOnlyAfterProcessLocalCompletion(t *testing.T) {
	rt, outbox, fence := joinInitialClassAssignmentCharacter(t)
	if state, _ := rt.characters.State(fence.EntityID); state.ClassID != "" { t.Fatalf("fresh class=%q", state.ClassID) }

	if err := rt.EnqueueFencedInitialClassAssignment(fence, classid.Oathguard); err != nil { t.Fatal(err) }
	report := rt.Step(2, 50*time.Millisecond)
	if len(report.CommandErrors) != 0 { t.Fatalf("prepare errors=%#v", report.CommandErrors) }
	...
	if len(report.CommandErrors) != 1 || !errors.Is(report.CommandErrors[0].Err, ErrInitialClassAssignmentPending) { t.Fatalf("duplicate errors=%#v", report.CommandErrors) }
	...
	if len(report.CommandErrors) != 0 { t.Fatalf("completion errors=%#v", report.CommandErrors) }
	state, ok = rt.characters.State(fence.EntityID)
	...
	if len(report.CommandErrors) != 1 || !errors.Is(report.CommandErrors[0].Err, character.ErrClassAlreadyAssigned) { t.Fatalf("transfer errors=%#v", report.CommandErrors) }
	if state, _ := rt.characters.State(fence.EntityID); state.ClassID != classid.Oathguard { t.Fatalf("transfer changed class=%q", state.ClassID) }
}
...
func TestInitialClassAssignmentRejectsStaleOwnershipAndMissingPersistence(t *testing.T) {
	...
	if err := rt.EnqueueFencedInitialClassAssignment(fence, classid.Ranger); err != nil { t.Fatal(err) }
	report := rt.Step(2, 50*time.Millisecond)
	if len(report.CommandErrors) != 1 || !errors.Is(report.CommandErrors[0].Err, ErrCharacterOwnershipFenceStale) { t.Fatalf("stale errors=%#v", report.CommandErrors) }
	if state, _ := rt.characters.State(fence.EntityID); state.ClassID != "" { t.Fatalf("stale owner changed class=%q", state.ClassID) }

	plain, plainFence, _ := joinOwnedIdentitySession(t)
	if err := plain.EnqueueFencedInitialClassAssignment(plainFence, classid.Ranger); err != nil { t.Fatal(err) }
	report = plain.Step(2, 50*time.Millisecond)
	if len(report.CommandErrors) != 1 || !errors.Is(report.CommandErrors[0].Err, ErrInitialClassAssignmentRequiresPersistence) { t.Fatalf("missing persistence errors=%#v", report.CommandErrors) }
}

func TestInitialClassCompletionAfterLeaveDoesNotPersistClass(t *testing.T) {
	rt, outbox, fence := joinInitialClassAssignmentCharacter(t)
...
func joinInitialClassAssignmentCharacter(t *testing.T) (*Runtime, *characterstate.Outbox, SessionOwnershipFence) {
	t.Helper()
	outbox, err := characterstate.NewOutbox(8); if err != nil { t.Fatal(err) }
	...
	cfg := DefaultConfig(); cfg.SnapshotEveryTicks = 1000; cfg.CharacterStateAutosaveEveryTicks = 0
	rt := New(sim, cfg, WithCharacterStateOutbox(outbox, characterStateTestWorld))
	identity, err := characteridentity.NewTrusted("character:initial-class"); if err != nil { t.Fatal(err) }
	conn := session.NewQueueConnection(32, 32)
	sess, err := session.NewWithCharacterIdentity(1, 1, identity, 64, conn); if err != nil { t.Fatal(err) }
	request := JoinRequest{Session: sess, Entity: world.EntityState{ID: 1, Kind: world.EntityPlayer, Transform: world.Transform{Position: world.Position{Layer: 0}}}, Speed: 6, Radius: 0.35, MaxStepHeight: 0.5}
	result := make(chan struct{ fence SessionOwnershipFence; err error }, 1)
	go func() {
		fence, err := rt.AwaitJoinOwned(nil, request)
		result <- struct{ fence SessionOwnershipFence; err error }{fence: fence, err: err}
	}()
	...
	got := <-result
	if got.err != nil || !got.fence.Valid() { t.Fatalf("fence=%#v err=%v", got.fence, got.err) }
