...
func TestStepRetriesPendingLegacyClassResourceFeedback(t *testing.T) {
	...
	if _, err := rt.characters.GainClassResource(1, classresource.Resolve, 8); err != nil {
		t.Fatal(err)
	}
	...
	if got := len(rt.pendingClassMessages[s.ID]); got != 1 {
		t.Fatalf("pending class resource messages=%d want=1", got)
	}
	if len(connection.sent) != 0 {
		t.Fatalf("backpressured resource state was sent: %d envelopes", len(connection.sent))
	}

	report := rt.Step(2, 50*time.Millisecond)
	if len(report.DeliveryErrors) != 0 {
		t.Fatalf("retry delivery errors=%#v", report.DeliveryErrors)
	}
	if _, ok := rt.pendingClassMessages[s.ID]; ok {
		t.Fatalf("pending class resource queue not drained: %#v", rt.pendingClassMessages[s.ID])
	}
	...
	for _, envelope := range connection.sent {
		...
		resourceMessages++
		if envelope.Delivery != protocol.DeliveryReliableOrdered || envelope.ServerTick != 2 {
			t.Fatalf("resource envelope=%#v, want reliable tick 2", envelope)
		}
	...
	if resourceMessages != 1 {
		t.Fatalf("resource messages=%d want=1; sent=%#v", resourceMessages, connection.sent)
	}
}
