...
func TestEquipmentCommandMovesTrainingBladeAndReplicatesAuthoritativeTruth(t *testing.T) {
	...
	if inventoryAfterEquip.Revision != 4 { t.Fatalf("inventory revision after equip=%d want=4", inventoryAfterEquip.Revision) }
	...
	if inventoryAfterUnequip.Revision != 5 { t.Fatalf("inventory revision after unequip=%d want=5", inventoryAfterUnequip.Revision) }
	bladeQuantity := uint32(0)
	for _, stack := range inventoryAfterUnequip.Items {
		if stack.ArchetypeID == "item_training_blade" { bladeQuantity = stack.Quantity }
	}
