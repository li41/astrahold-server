...
func TestServerCollectorSummarizesTickAOIAndReplication(t *testing.T) {
	...
	collector.RecordStep(worldruntime.StepReport{
		...
		Metrics: worldruntime.StepMetrics{
			...
			InitialVitalsGlobalBudget:          8,
			InitialVitalsGlobalSelected:        7,
			InitialVitalsGlobalBudgetExhausted: true,
	...
	report := collector.Finish(ScenarioCrowd, 3)
	...
	if report.Lifecycle.SpawnCandidates != 8 || report.Lifecycle.SpawnSelected != 6 || report.Lifecycle.SpawnDeferred != 2 ||
		...
		report.Lifecycle.InitialVitalsBudgetPerTick != 8 || report.Lifecycle.InitialVitalsSelected != 7 || report.Lifecycle.InitialVitalsBudgetExhaustions != 1 || report.Lifecycle.MaxInitialVitalsSelectedPerTick != 7 {
