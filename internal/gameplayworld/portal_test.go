...
func TestCastleSandboxWorldIsValid(t *testing.T) {
	...
	if loaded.Definition.Revision != "s5c-large-castle-001" {
		t.Fatalf("revision = %q, want s5c-large-castle-001", loaded.Definition.Revision)
	}
	if len(loaded.Definition.Surfaces) != 15 || len(loaded.Definition.Portals) != 4 || len(loaded.Definition.Blockers) != 33 || len(loaded.Definition.Gates) != 1 {
