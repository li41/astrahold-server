...
func (i *Inventory) AddInstance(instance iteminstance.Instance) error {
	...
	if instanceID == "" || archetypeID == "" || instanceID != instance.ID || archetypeID != instance.ItemArchetypeID { return ErrInvalidInstance }
	if _, exists := i.instances[instanceID]; exists || i.instanceEquipped(instanceID) { return ErrInstanceExists }
	if i.unequippedEntryCount() >= i.maxStacks { return ErrFull }
...
func (i *Inventory) InstanceSnapshot() []iteminstance.Instance {
	...
	for _, instance := range i.instances { out = append(out, cloneInstance(instance)) }
	sort.Slice(out, func(a, b int) bool { return out[a].ID < out[b].ID })
	return out
}

func (i *Inventory) EquippedInstance(slot EquipmentSlot) (iteminstance.Instance, bool) {
	if i == nil || !ValidEquipmentSlot(slot) { return iteminstance.Instance{}, false }
	instance := i.equippedInstances[slot]
	if instance.ID == "" { return iteminstance.Instance{}, false }
	return cloneInstance(instance), true
}
...
func (i *Inventory) MainHandInstance() (iteminstance.Instance, bool) { return i.EquippedInstance(SlotMainHand) }
func (i *Inventory) OffHandInstance() (iteminstance.Instance, bool) { return i.EquippedInstance(SlotOffHand) }

func (i *Inventory) EquipInstance(slot EquipmentSlot, instanceID iteminstance.ID) error {
	if i == nil { return ErrInstanceNotFound }
	...
	instance, ok := i.instances[instanceID]
	if !ok { return ErrInstanceNotFound }
	delete(i.instances, instanceID)
	i.equippedInstances[slot] = cloneInstance(instance)
	i.revision++
	i.equipmentRevision++
...
func (i *Inventory) UnequipInstance(slot EquipmentSlot) (iteminstance.Instance, error) {
	if i == nil { return iteminstance.Instance{}, ErrEquipmentSlotEmpty }
	if !ValidEquipmentSlot(slot) { return iteminstance.Instance{}, ErrInvalidEquipmentSlot }
	instance := i.equippedInstances[slot]
	if instance.ID == "" { return iteminstance.Instance{}, ErrEquipmentSlotEmpty }
	if i.unequippedEntryCount() >= i.maxStacks { return iteminstance.Instance{}, ErrFull }
...
func (i *Inventory) EquipMainHandInstance(instanceID iteminstance.ID) error { return i.EquipInstance(SlotMainHand, instanceID) }
func (i *Inventory) UnequipMainHandInstance() (iteminstance.Instance, error) { return i.UnequipInstance(SlotMainHand) }
func (i *Inventory) EquipOffHandInstance(instanceID iteminstance.ID) error { return i.EquipInstance(SlotOffHand, instanceID) }
func (i *Inventory) UnequipOffHandInstance() (iteminstance.Instance, error) { return i.UnequipInstance(SlotOffHand) }

func cloneInstance(instance iteminstance.Instance) iteminstance.Instance {
	copy := instance
	if len(instance.Affixes) > 0 { copy.Affixes = append(copy.Affixes[:0:0], instance.Affixes...) }
	return copy
}
