...
func New(maxStacks int) *Inventory { return NewWithWeightPolicy(maxStacks, WeightPolicy{}) }
...
func (i *Inventory) Remove(archetypeID string, quantity uint32) error {
	...
	if current < quantity { return ErrInsufficient }
	remaining := current - quantity
	if remaining == 0 { delete(i.stacks, archetypeID) } else { i.stacks[archetypeID] = remaining }
...
func (i *Inventory) Exchange(removeArchetypeID string, removeQuantity uint32, addArchetypeID string, addQuantity uint32) error {
	...
	if removeArchetypeID == "" || addArchetypeID == "" { return ErrInvalidArchetype }
	if removeQuantity == 0 || addQuantity == 0 { return ErrInvalidQuantity }
	removeCurrent := i.stacks[removeArchetypeID]
	if removeCurrent < removeQuantity { return ErrInsufficient }
	removedWeight := i.weightFor(removeArchetypeID, removeQuantity)
	baseWeight := uint64(0)
	if removedWeight < i.currentWeight { baseWeight = i.currentWeight - removedWeight }
	addedWeight := i.weightFor(addArchetypeID, addQuantity)
	if i.maxWeight > 0 && (addedWeight > i.maxWeight || baseWeight > i.maxWeight-addedWeight) { return ErrWeightExceeded }
	if removeArchetypeID == addArchetypeID {
		final := uint64(removeCurrent-removeQuantity) + uint64(addQuantity)
		if final > math.MaxUint32 { return ErrQuantityOverflow }
...
func (i *Inventory) Snapshot() []Stack {
	if i == nil || len(i.stacks) == 0 { return nil }
