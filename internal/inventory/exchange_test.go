...
func TestExchangeConsumesAndGrantsWithSingleRevision(t *testing.T) {
	inv := New(4)
	if err := inv.Add("item_gray_wolf_pelt", 1); err != nil {
		t.Fatal(err)
	}
	if err := inv.Add("item_minor_healing_potion", 5); err != nil {
		t.Fatal(err)
	}
	before := inv.Revision()

	if err := inv.Exchange("item_gray_wolf_pelt", 1, "item_minor_healing_potion", 1); err != nil {
		t.Fatal(err)
	}
	if got := inv.Quantity("item_gray_wolf_pelt"); got != 0 {
	...
	if got := inv.Revision(); got != before+1 {
...
func TestExchangeInsufficientLeavesInventoryUnchanged(t *testing.T) {
	...
	if got := inv.Quantity("item_minor_healing_potion"); got != 5 {
		t.Fatalf("potion quantity = %d, want 5", got)
	}
	if got := inv.Revision(); got != before {
...
func TestExchangeCanReuseStackSlotFreedByCost(t *testing.T) {
	...
	if got := inv.Quantity("item_minor_healing_potion"); got != 1 {
		t.Fatalf("potion quantity = %d, want 1", got)
	}
}
