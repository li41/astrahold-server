...
func validateState(state State) error {
	...
	if state.Defeated {
		if state.HP != 0 { return ErrInvalidState }
	} else if state.HP == 0 { return ErrInvalidState }
	return nil
}
...
func (s *Service) States() []State {
	...
	for _, state := range s.states { out = append(out, state) }
...
func (s *Service) GainClassResource(id world.EntityID, resourceID classresource.ID, amount uint32) (State, error) {
	...
	if !ok { return State{}, ErrCharacterNotFound }
	if state.Defeated { return state, ErrCharacterDefeated }
...
func (s *Service) GainClassResourceProgress(id world.EntityID, resourceID classresource.ID, amount uint32) (State, bool, error) {
	state, ok := s.states[id]
	if !ok { return State{}, false, ErrCharacterNotFound }
	if state.Defeated { return state, false, ErrCharacterDefeated }
	...
	if gained > 0 {
		missing := state.MaxClassResource - state.ClassResource
		if gained >= missing { state.ClassResource = state.MaxClassResource; state.ClassResourceProgress = 0 } else { state.ClassResource += gained }
	}
	s.states[id] = state
...
func (s *Service) GainTargetResource(sourceID, targetID world.EntityID, resourceID targetresource.ID, amount, max uint32, tick, nextReadyTick uint64) (targetresource.State, bool, error) {
	...
	if !ok { return targetresource.State{}, false, ErrCharacterNotFound }
	if source.Defeated { return targetresource.State{}, false, ErrCharacterDefeated }
...
func (s *Service) SpendMP(id world.EntityID, amount uint32) (State, error) {
	...
	if state.MP < amount { return state, ErrInsufficientResource }
	state.MP -= amount
	s.states[id] = state
	return state, nil
}
...
func (s *Service) RestoreHP(id world.EntityID, amount uint32) (State, error) {
	...
	if !ok { return State{}, ErrCharacterNotFound }
	if state.Defeated { return state, ErrCharacterDefeated }
	if state.HP >= state.MaxHP { return state, ErrResourceFull }
...
func (s *Service) RestoreMP(id world.EntityID, amount uint32) (State, error) {
	...
	if !ok { return State{}, ErrCharacterNotFound }
	if state.Defeated { return state, ErrCharacterDefeated }
	if state.MP >= state.MaxMP { return state, ErrResourceFull }
...
func (s *Service) ApplyDamage(id world.EntityID, amount uint32) (State, error) {
	...
	if !ok { return State{}, ErrCharacterNotFound }
	if state.Defeated { return state, ErrCharacterDefeated }
	if amount >= state.HP { state.HP = 0; state.Defeated = true } else { state.HP -= amount }
	s.states[id] = state
	return state, nil
}
