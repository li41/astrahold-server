...
func TestRegisterStateInitializesResolveDefinition(t *testing.T) {
	...
	if err := service.RegisterState(State{EntityID: entityID, ClassResourceID: classresource.Resolve, HP: 1000, MaxHP: 1000}); err != nil { t.Fatal(err) }
...
func TestResourceGainClamps(t *testing.T) {
	service, err := NewService(1000)
	if err != nil { t.Fatal(err) }
	...
	if err != nil { t.Fatal(err) }
	if state.ClassResource != 8 { t.Fatalf("resolve = %d, want 8", state.ClassResource) }
	state, err = service.GainClassResource(entityID, classresource.Resolve, 200)
	if err != nil { t.Fatal(err) }
	if state.ClassResource != 100 { t.Fatalf("resolve = %d, want clamp 100", state.ClassResource) }
}
...
func TestClassResourceRejectsMismatchWithoutMutation(t *testing.T) {
	...
	if state.ClassResource != 0 { t.Fatalf("resolve mutated to %d", state.ClassResource) }
}
...
func TestRegisterStateWithoutRuntimeResourceHasNone(t *testing.T) {
	...
	if state.ClassResourceID != classresource.Empty || state.ClassResource != 0 || state.MaxClassResource != 0 || state.ClassResourceProgress != 0 {
		t.Fatalf("unexpected resource = %q %d/%d progress=%d", state.ClassResourceID, state.ClassResource, state.MaxClassResource, state.ClassResourceProgress)
	}
}
