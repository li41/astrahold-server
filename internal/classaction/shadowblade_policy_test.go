...
func TestShadowbladeDualBladePolicy(t *testing.T) {
	...
	if !ok {
		t.Fatal("missing Shadowblade dual blade policy")
	}
	...
	if policy.HitResource != "" || policy.AcceptedResource != "" || policy.HitProgressResource != "" || policy.TargetSpend.ResourceID != "" {
...
func TestShadowbladeFlawExecutePolicyUsesHighestAvailableCanonicalTier(t *testing.T) {
	...
	if !ok {
		t.Fatal("missing Shadowblade Flaw Execute policy")
	}
	if policy.TargetSpend.ResourceID != targetresource.Flaw {
		t.Fatalf("policy=%+v", policy)
	}
