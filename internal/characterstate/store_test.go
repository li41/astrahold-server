...
func TestStoreCreateUpdateReopenAndCAS(t *testing.T) {
	...
	if first.Revision != 1 || first.CharacterID != identity.ID || first.Snapshot != firstSnapshot {
	...
	if second.Revision != 2 || second.Snapshot != updatedSnapshot {
		t.Fatalf("second=%#v", second)
	}

	if _, err := store.Save(identity, 1, firstSnapshot); !errors.Is(err, ErrRevisionConflict) {
		t.Fatalf("stale save err=%v", err)
	}
	current, ok, err := store.Load(identity)
	if err != nil || !ok || current != second {
	...
	if err != nil || !ok || loaded != second {
		t.Fatalf("reopened=%#v ok=%v err=%v", loaded, ok, err)
	}
}
...
func TestStoreCreateOnlyConflictDoesNotOverwrite(t *testing.T) {
	...
	first, err := store.Save(identity, 0, testSnapshot())
	if err != nil {
		t.Fatal(err)
	}
	changed := testSnapshot()
	changed.HP = 500
	if _, err := store.Save(identity, 0, changed); !errors.Is(err, ErrRevisionConflict) {
		t.Fatalf("create-only err=%v", err)
	}
...
func TestStoreRejectsEphemeralIdentity(t *testing.T) {
	...
	if _, err := store.Save(ephemeral, 0, testSnapshot()); !errors.Is(err, ErrIdentityNotDurable) {
		t.Fatalf("save err=%v", err)
	}
	if _, _, err := store.Load(ephemeral); !errors.Is(err, ErrIdentityNotDurable) {
...
func TestStoreFailsClosedOnUnknownTrailingOrMismatchedRecord(t *testing.T) {
	for _, tc := range []struct {
		...
		},
		{
			name: "mismatched id",
			mutate: func(wire map[string]any) []byte {
				wire["character_id"] = "character:other"
				data, _ := json.Marshal(wire)
				return append(data, '\n')
			},
		},
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if _, err := store.Save(identity, 0, testSnapshot()); err != nil {
				t.Fatal(err)
			}
...
func TestStoreRevisionOverflowDoesNotAdvanceTruth(t *testing.T) {
	...
	if _, err := store.Save(identity, ^uint64(0), snapshot); !errors.Is(err, ErrRevisionOverflow) {
		t.Fatalf("overflow err=%v", err)
	}
	loaded, ok, err := store.Load(identity)
	if err != nil || !ok || loaded.Revision != ^uint64(0) {
		t.Fatalf("loaded=%#v ok=%v err=%v", loaded, ok, err)
	}
}
...
func TestStoreUsesHashedFilenamesAndSeparatesCharacters(t *testing.T) {
	...
	if _, err := store.Save(firstIdentity, 0, testSnapshot()); err != nil {
		t.Fatal(err)
	}
	secondSnapshot := testSnapshot()
	secondSnapshot.HP = 600
	if _, err := store.Save(secondIdentity, 0, secondSnapshot); err != nil {
		t.Fatal(err)
	}
	if filepath.Base(store.recordPath(firstIdentity.ID)) == filepath.Base(store.recordPath(secondIdentity.ID)) {
		t.Fatal("distinct character ids mapped to same record path")
	}
