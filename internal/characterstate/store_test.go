...
func TestStoreCreateUpdateReopenAndCAS(t *testing.T) {
	...
	store, err := Open(root); if err != nil { t.Fatal(err) }
...
func TestStoreMigratesV2RecordToFullMP(t *testing.T) {
	...
	if loaded.Snapshot.MP != LegacyDefaultMaxMP || loaded.Snapshot.MaxMP != LegacyDefaultMaxMP { t.Fatalf("migrated mp=%d/%d", loaded.Snapshot.MP, loaded.Snapshot.MaxMP) }
	if loaded.Snapshot.PrimaryStats != characterstats.DefaultPrimary() { t.Fatalf("migrated stats=%#v", loaded.Snapshot.PrimaryStats) }
	if loaded.Snapshot.World.MapID != LegacyDefaultMapID { t.Fatalf("migrated map=%q want=%q", loaded.Snapshot.World.MapID, LegacyDefaultMapID) }
}
...
func TestStoreRejectsInvalidSnapshots(t *testing.T) {
	...
	tests := []Snapshot{{}, func() Snapshot { s := testSnapshot(); s.World.GameplaySHA256 = "BAD"; return s }(), func() Snapshot { s := testSnapshot(); s.MaxHP = 0; return s }(), func() Snapshot { s := testSnapshot(); s.HP = s.MaxHP + 1; return s }(), func() Snapshot { s := testSnapshot(); s.MaxMP = 0; return s }(), func() Snapshot { s := testSnapshot(); s.MP = s.MaxMP + 1; return s }(), func() Snapshot { s := testSnapshot(); s.Defeated = true; s.HP = 1; return s }(), func() Snapshot { s := testSnapshot(); s.HP = 0; s.Defeated = false; return s }(), func() Snapshot { s := testSnapshot(); s.Position.X = float32(math.NaN()); return s }(), func() Snapshot { s := testSnapshot(); s.Yaw = float32(math.Inf(1)); return s }(), func() Snapshot { s := testSnapshot(); s.PrimaryStats.Charisma = 9; return s }()}
	for i, snapshot := range tests { if _, err := store.Save(identity, 0, snapshot); !errors.Is(err, ErrInvalidSnapshot) { t.Fatalf("case=%d err=%v", i, err) } }
}
...
func TestStoreFailsClosedOnUnknownTrailingOrMismatchedRecord(t *testing.T) {
	for _, tc := range []struct { name string; mutate func(map[string]any) []byte }{
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			path := store.recordPath(identity.ID); data, err := os.ReadFile(path); if err != nil { t.Fatal(err) }
...
func TestStoreRevisionOverflowDoesNotAdvanceTruth(t *testing.T) {
	...
	loaded, ok, err := store.Load(identity); if err != nil || !ok || loaded.Revision != ^uint64(0) { t.Fatalf("loaded=%#v ok=%v err=%v", loaded, ok, err) }
}

func TestStoreUsesHashedFilenamesAndSeparatesCharacters(t *testing.T) {
	store, err := Open(t.TempDir()); if err != nil { t.Fatal(err) }
	firstIdentity := trusted(t, "character:alpha"); secondIdentity := trusted(t, "character:beta")
