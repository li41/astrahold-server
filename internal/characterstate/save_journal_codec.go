...
func saveJournalWireToSnapshot(schemaVersion uint16, wire saveJournalWireSnapshot) (Snapshot, error) {
	...
	if schemaVersion < MapSaveJournalSchemaVersion && wire.MapID != "" { return Snapshot{}, ErrInvalidSnapshot }
	if schemaVersion >= MapSaveJournalSchemaVersion && !validMapID(wire.MapID) { return Snapshot{}, ErrInvalidSnapshot }
	...
	primaryStats, err := primaryStatsFromWire(storeSchema, wire.PrimaryStats); if err != nil { return Snapshot{}, err }
	mapID := LegacyDefaultMapID
	if schemaVersion >= MapSaveJournalSchemaVersion { mapID = wire.MapID }
...
func validateNewSaveIntent(intent SaveIntent) error {
	...
	return validateSnapshotV14(intent.Snapshot)
}
...
func validateDecodedSaveIntent(schemaVersion uint16, intent SaveIntent) error {
	...
	case MapSaveJournalSchemaVersion:
		if !intent.Snapshot.Inventory.Initialized { return ErrInvalidSnapshot }
		if err := validateSnapshotV14(intent.Snapshot); err != nil { return err }
	default:
