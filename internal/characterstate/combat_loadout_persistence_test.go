...
func TestStoreCurrentSchemaRoundTripsCombatLoadout(t *testing.T) {
	...
	if err != nil { t.Fatal(err) }
	if record.SchemaVersion != SchemaVersion || record.Snapshot.CombatLoadout != snapshot.CombatLoadout { t.Fatalf("record=%#v", record) }
	loaded, ok, err := store.Load(identity)
	if err != nil || !ok { t.Fatalf("loaded=%#v ok=%v err=%v", loaded, ok, err) }
	if loaded.SchemaVersion != SchemaVersion || loaded.Snapshot.CombatLoadout != snapshot.CombatLoadout { t.Fatalf("loaded=%#v", loaded) }
}
...
func TestStoreV6MigratesToEmptyCombatLoadout(t *testing.T) {
	...
	if loaded.Snapshot.CombatLoadout != (skillloadout.Slots{}) { t.Fatalf("legacy combat loadout=%v", loaded.Snapshot.CombatLoadout) }
}

func TestStoreRejectsCombatLoadoutThatDoesNotMatchSchema(t *testing.T) {
	for _, tc := range []struct { name string; schema uint16; ids []string }{
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			snapshot := testSnapshot()
			wire := wireRecord{SchemaVersion: tc.schema, CharacterID: string(identity.ID), Revision: 1, WorldID: snapshot.World.WorldID, WorldRevision: snapshot.World.Revision, GameplaySHA256: snapshot.World.GameplaySHA256, HP: snapshot.HP, MaxHP: snapshot.MaxHP, MP: snapshot.MP, MaxMP: snapshot.MaxMP, Defeated: snapshot.Defeated, X: snapshot.Position.X, Y: snapshot.Position.Y, Z: snapshot.Position.Z, Layer: snapshot.Position.Layer, Yaw: snapshot.Yaw, Inventory: snapshot.Inventory, CombatLoadout: tc.ids}
			data, err := json.Marshal(wire); if err != nil { t.Fatal(err) }
			if err := os.WriteFile(store.recordPath(identity.ID), append(data, '\n'), 0o600); err != nil { t.Fatal(err) }
...
func TestSaveJournalCurrentSchemaRoundTripsCombatLoadout(t *testing.T) {
	...
	if len(records) != 1 || records[0].RecordID != record.RecordID || records[0].Intent.Snapshot.CombatLoadout != snapshot.CombatLoadout { t.Fatalf("records=%#v", records) }
}
...
func TestSaveJournalV5MigratesToEmptyCombatLoadout(t *testing.T) {
	...
	wire := saveJournalWireRecord{SchemaVersion: ClassSaveJournalSchemaVersion, RecordID: 1, ExpectedRevision: 0, IntentID: 1, CharacterID: string(trusted(t, "character:journal-loadout-v5").ID), Snapshot: wireSnapshot}
	...
	if intent.Snapshot.CombatLoadout != (skillloadout.Slots{}) { t.Fatalf("legacy combat loadout=%v", intent.Snapshot.CombatLoadout) }
}
...
func TestSaveJournalRejectsCombatLoadoutThatDoesNotMatchSchema(t *testing.T) {
	for _, tc := range []struct { name string; schema uint16; ids []string }{
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			wire := saveJournalWireRecord{SchemaVersion: tc.schema, RecordID: 1, ExpectedRevision: 0, IntentID: 1, CharacterID: string(trusted(t, "character:journal-loadout-corrupt").ID), Snapshot: wireSnapshot}
			payload, err := json.Marshal(wire); if err != nil { t.Fatal(err) }
...
func TestSaveJournalRejectsInvalidCombatLoadoutBeforeAppend(t *testing.T) {
	journal, err := OpenSaveJournal(filepath.Join(t.TempDir(), "invalid-loadout.journal")); if err != nil { t.Fatal(err) }
	defer journal.Close()
	snapshot := testSnapshot()
	snapshot.CombatLoadout = skillloadout.Slots{skillcatalog.HeavyStrike, "", skillcatalog.FireBolt}
