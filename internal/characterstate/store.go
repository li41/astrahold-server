...
type WorldRef struct { WorldID, Revision, GameplaySHA256 string }
...
type Record struct { SchemaVersion uint16; CharacterID characteridentity.ID; Revision uint64; Snapshot Snapshot }
type Store struct { mu sync.Mutex; root string }
...
type wireDefeatedRespawn struct {
	...
	X float32 `json:"x"`; Y float32 `json:"y"`; Z float32 `json:"z"`; Layer world.LayerID `json:"layer"`
...
type wireRecord struct {
	...
	HP uint32 `json:"hp"`; MaxHP uint32 `json:"max_hp"`; MP uint32 `json:"mp,omitempty"`; MaxMP uint32 `json:"max_mp,omitempty"`
	Defeated bool `json:"defeated"`
	X float32 `json:"x"`; Y float32 `json:"y"`; Z float32 `json:"z"`; Layer world.LayerID `json:"layer"`; Yaw float32 `json:"yaw"`
...
// Load accepts v1-v5. v1/v2 predate MP; v1-v3 predate inventory; v4 has MainHand only;
// v5 adds durable OffHand while keeping old v4 records readable with an empty OffHand.
...
func (s *Store) Save(identity characteridentity.Binding, expectedRevision uint64, snapshot Snapshot) (Record, error) {
	...
	if err != nil { return Record{}, err }
	currentRevision := uint64(0); if exists { currentRevision = current.Revision }
	if currentRevision != expectedRevision { return Record{}, fmt.Errorf("%w: character=%s expected=%d current=%d", ErrRevisionConflict, identity.ID, expectedRevision, currentRevision) }
...
func (s *Store) loadLocked(identity characteridentity.Binding) (Record, bool, error) {
	...
	if wire.SchemaVersion < LegacySchemaVersion || wire.SchemaVersion > SchemaVersion || wire.CharacterID != string(identity.ID) || wire.Revision == 0 { return Record{}, false, ErrCorruptRecord }
	...
	if wire.SchemaVersion < ResourceSchemaVersion { mp, maxMP = LegacyDefaultMaxMP, LegacyDefaultMaxMP }
	...
	record := Record{SchemaVersion: wire.SchemaVersion, CharacterID: characteridentity.ID(wire.CharacterID), Revision: wire.Revision, Snapshot: Snapshot{
		...
		Position: world.Position{X: wire.X, Y: wire.Y, Z: wire.Z, Layer: wire.Layer}, Yaw: wire.Yaw, Inventory: inventoryState,
	}}
	if wire.DefeatedRespawn != nil {
		record.Snapshot.Respawn = DefeatedRespawn{Context: wire.DefeatedRespawn.Context, SpawnPointID: wire.DefeatedRespawn.SpawnPointID, SpawnClass: wire.DefeatedRespawn.SpawnClass,
			Position: world.Position{X: wire.DefeatedRespawn.X, Y: wire.DefeatedRespawn.Y, Z: wire.DefeatedRespawn.Z, Layer: wire.DefeatedRespawn.Layer}, RemainingTicks: wire.DefeatedRespawn.RemainingTicks, CheckpointID: wire.DefeatedRespawn.CheckpointID}
	}
	if err := validateSnapshotBase(record.Snapshot); err != nil { return Record{}, false, fmt.Errorf("%w: %v", ErrCorruptRecord, err) }
	if wire.SchemaVersion >= RespawnSchemaVersion { if err := validateRespawnSnapshot(record.Snapshot); err != nil { return Record{}, false, fmt.Errorf("%w: %v", ErrCorruptRecord, err) } }
	if wire.SchemaVersion >= ResourceSchemaVersion && (record.Snapshot.MaxMP == 0 || record.Snapshot.MP > record.Snapshot.MaxMP) { return Record{}, false, fmt.Errorf("%w: %v", ErrCorruptRecord, ErrInvalidSnapshot) }
	if wire.SchemaVersion >= InventorySchemaVersion { if err := validateInventoryState(record.Snapshot.Inventory); err != nil { return Record{}, false, fmt.Errorf("%w: %v", ErrCorruptRecord, err) } }
	return record, true, nil
}

func (s *Store) writeLocked(record Record) error {
	inventoryState, err := CanonicalInventoryState(record.Snapshot.Inventory); if err != nil { return err }
	wire := wireRecord{SchemaVersion: SchemaVersion, CharacterID: string(record.CharacterID), Revision: record.Revision,
		...
		X: record.Snapshot.Position.X, Y: record.Snapshot.Position.Y, Z: record.Snapshot.Position.Z, Layer: record.Snapshot.Position.Layer, Yaw: record.Snapshot.Yaw, Inventory: inventoryState}
	if record.Snapshot.Defeated {
		respawn := record.Snapshot.Respawn
		wire.DefeatedRespawn = &wireDefeatedRespawn{Context: respawn.Context, SpawnPointID: respawn.SpawnPointID, SpawnClass: respawn.SpawnClass,
			X: respawn.Position.X, Y: respawn.Position.Y, Z: respawn.Position.Z, Layer: respawn.Position.Layer, RemainingTicks: respawn.RemainingTicks, CheckpointID: respawn.CheckpointID}
	}
	data, err := json.Marshal(wire); if err != nil { return err }; data = append(data, '\n')
	tmp, err := os.CreateTemp(s.root, ".character-state.tmp-"); if err != nil { return err }
...
func (s *Store) recordPath(id characteridentity.ID) string { sum := sha256.Sum256([]byte(id)); return filepath.Join(s.root, fmt.Sprintf("%x.json", sum[:])) }
func validateTrustedIdentity(identity characteridentity.Binding) error { if !identity.Valid() || identity.Assurance != characteridentity.AssuranceTrusted { return ErrIdentityNotDurable }; return nil }
func validateSnapshotV5(snapshot Snapshot) error { if err := validateSnapshotV3(snapshot); err != nil { return err }; return validateInventoryState(snapshot.Inventory) }
// validateSnapshotV4 remains as an internal compatibility alias for existing focused tests/helpers.
func validateSnapshotV4(snapshot Snapshot) error { return validateSnapshotV5(snapshot) }
...
func validateRespawnSnapshot(snapshot Snapshot) error {
	if snapshot.Defeated {
		if !validDeathContext(snapshot.Respawn.Context) || snapshot.Respawn.SpawnPointID == "" || !validSpawnClass(snapshot.Respawn.SpawnClass) || !finitePosition(snapshot.Respawn.Position) { return ErrInvalidSnapshot }
		return nil
	}
...
func validateSnapshotBase(snapshot Snapshot) error {
	...
	if snapshot.Defeated { if snapshot.HP != 0 { return ErrInvalidSnapshot } } else if snapshot.HP == 0 { return ErrInvalidSnapshot }
	if !finitePosition(snapshot.Position) || !finite32(snapshot.Yaw) { return ErrInvalidSnapshot }
	return nil
}
func validDeathContext(context respawnpolicy.DeathContext) bool { switch context { case respawnpolicy.DeathContextPvE, respawnpolicy.DeathContextPvP, respawnpolicy.DeathContextSiege: return true; default: return false } }
func validSpawnClass(class respawnpolicy.SpawnClass) bool { switch class { case respawnpolicy.SpawnClassSafe, respawnpolicy.SpawnClassCheckpoint, respawnpolicy.SpawnClassSiege: return true; default: return false } }
...
func validSHA256(value string) bool {
	...
	for i := 0; i < len(value); i++ { c := value[i]; if (c >= '0' && c <= '9') || (c >= 'a' && c <= 'f') { continue }; return false }
	return true
}
