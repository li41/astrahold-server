...
const (
	...
	PrimaryStatsSaveJournalSchemaVersion  uint16 = 10
	SaveJournalSchemaVersion              uint16 = PrimaryStatsSaveJournalSchemaVersion
...
func (j *SaveJournal) RecordsAfter(checkpoint SaveCheckpoint, limit int) ([]SaveJournalRecord, error) {
	...
	offset := checkpoint.Offset; expectedRecordID := checkpoint.RecordID + 1
	for len(out) < count {
...
func (s *SaveCheckpointStore) Load(journal *SaveJournal) (SaveCheckpoint, error) {
	...
	if errors.Is(err, os.ErrNotExist) { return journal.InitialCheckpoint(), nil }
	if err != nil { return SaveCheckpoint{}, err }
	...
	if err := decoder.Decode(&trailing); err != io.EOF {
		if err == nil { return SaveCheckpoint{}, fmt.Errorf("%w: trailing JSON value", ErrCorruptSaveCheckpoint) }
	...
	if err := journal.ValidateCheckpoint(checkpoint); err != nil { return SaveCheckpoint{}, err }
...
func (s *SaveCheckpointStore) Save(journal *SaveJournal, record SaveJournalRecord) (SaveCheckpoint, error) {
	...
	tmp, err := os.CreateTemp(dir, "."+filepath.Base(s.path)+".tmp-"); if err != nil { return SaveCheckpoint{}, err }
	tmpName := tmp.Name(); cleanup := func() { _ = tmp.Close(); _ = os.Remove(tmpName) }
...
func (j *SaveJournal) checkpointForRecord(record SaveJournalRecord) (SaveCheckpoint, error) {
	j.mu.Lock(); defer j.mu.Unlock()
	if j.closed { return SaveCheckpoint{}, ErrSaveJournalClosed }
	if record.RecordID == 0 || record.RecordID > j.lastRecordID { return SaveCheckpoint{}, ErrSaveCheckpointAhead }
	expectedEnd := j.recordEnds[record.RecordID-1]
	if record.EndOffset != expectedEnd { return SaveCheckpoint{}, ErrSaveCheckpointOffsetMismatch }
...
func (j *SaveJournal) validateCheckpointLocked(checkpoint SaveCheckpoint) error {
	if checkpoint.JournalID != j.journalIDHex { return ErrSaveCheckpointJournalMismatch }
	if checkpoint.RecordID > j.lastRecordID { return ErrSaveCheckpointAhead }
	expectedOffset := saveJournalHeaderSize()
	if checkpoint.RecordID > 0 { expectedOffset = j.recordEnds[checkpoint.RecordID-1] }
	if checkpoint.Offset != expectedOffset { return fmt.Errorf("%w: got=%d want=%d", ErrSaveCheckpointOffsetMismatch, checkpoint.Offset, expectedOffset) }
	return nil
}

func (j *SaveJournal) openOrInitialize() error {
	stat, err := j.file.Stat(); if err != nil { return err }
	if stat.Size() == 0 {
		if _, err := rand.Read(j.journalID[:]); err != nil { return err }
		header := make([]byte, 0, saveJournalHeaderSize()); header = append(header, saveJournalMagic...); header = append(header, j.journalID[:]...)
		if _, err := j.file.WriteAt(header, 0); err != nil { return err }
		if err := j.file.Sync(); err != nil { return err }
		j.journalIDHex = hex.EncodeToString(j.journalID[:]); j.endOffset = saveJournalHeaderSize(); return nil
	}
	if stat.Size() < saveJournalHeaderSize() { return fmt.Errorf("%w: incomplete header", ErrCorruptSaveJournal) }
	header := make([]byte, saveJournalHeaderSize()); if _, err := j.file.ReadAt(header, 0); err != nil { return err }
	if !bytes.Equal(header[:len(saveJournalMagic)], saveJournalMagic) { return fmt.Errorf("%w: bad magic", ErrCorruptSaveJournal) }
	copy(j.journalID[:], header[len(saveJournalMagic):])
	allZero := true; for _, value := range j.journalID { if value != 0 { allZero = false; break } }
	if allZero { return fmt.Errorf("%w: empty journal id", ErrCorruptSaveJournal) }
	j.journalIDHex = hex.EncodeToString(j.journalID[:]); return j.scanExisting(stat.Size())
}

func (j *SaveJournal) scanExisting(size int64) error {
	offset := saveJournalHeaderSize(); expectedRecordID := uint64(1)
	for offset < size {
		remaining := size - offset; if remaining < 4 { return j.repairTornTail(offset) }
		var lengthBytes [4]byte; if _, err := j.file.ReadAt(lengthBytes[:], offset); err != nil { return err }
		length := binary.BigEndian.Uint32(lengthBytes[:]); if length == 0 || length > maxSaveJournalPayload { return fmt.Errorf("%w: invalid payload length=%d at offset=%d", ErrCorruptSaveJournal, length, offset) }
		frameEnd := offset + 4 + int64(length) + 4; if frameEnd > size { return j.repairTornTail(offset) }
		record, next, err := j.readRecordAtLocked(offset); if err != nil { return err }
		if next != frameEnd || record.RecordID != expectedRecordID { return fmt.Errorf("%w: record id got=%d want=%d", ErrCorruptSaveJournal, record.RecordID, expectedRecordID) }
		j.recordEnds = append(j.recordEnds, next); j.lastRecordID = record.RecordID; j.endOffset = next; offset = next; expectedRecordID++
	}
	if j.endOffset == 0 { j.endOffset = saveJournalHeaderSize() }
	return nil
}

func (j *SaveJournal) repairTornTail(offset int64) error {
	if err := j.file.Truncate(offset); err != nil { return err }
	if err := j.file.Sync(); err != nil { return err }
	j.endOffset = offset; j.repairedTail = true; return nil
}

func (j *SaveJournal) readRecordAtLocked(offset int64) (SaveJournalRecord, int64, error) {
	var lengthBytes [4]byte; if _, err := j.file.ReadAt(lengthBytes[:], offset); err != nil { return SaveJournalRecord{}, 0, err }
	length := binary.BigEndian.Uint32(lengthBytes[:]); if length == 0 || length > maxSaveJournalPayload { return SaveJournalRecord{}, 0, fmt.Errorf("%w: invalid payload length=%d", ErrCorruptSaveJournal, length) }
	payload := make([]byte, int(length)); if _, err := j.file.ReadAt(payload, offset+4); err != nil { return SaveJournalRecord{}, 0, err }
	var crcBytes [4]byte; if _, err := j.file.ReadAt(crcBytes[:], offset+4+int64(length)); err != nil { return SaveJournalRecord{}, 0, err }
	wantCRC := binary.BigEndian.Uint32(crcBytes[:]); gotCRC := crc32.Checksum(payload, saveCRCTable)
	if gotCRC != wantCRC { return SaveJournalRecord{}, 0, fmt.Errorf("%w: crc mismatch at offset=%d", ErrCorruptSaveJournal, offset) }
	recordID, expectedRevision, intent, err := decodeSaveJournalRecord(payload); if err != nil { return SaveJournalRecord{}, 0, err }
...
func decodeSaveJournalRecord(payload []byte) (uint64, uint64, SaveIntent, error) {
	...
	snapshot, err := saveJournalWireToSnapshot(wire.SchemaVersion, wire.Snapshot); if err != nil { return 0, 0, SaveIntent{}, fmt.Errorf("%w: snapshot: %v", ErrCorruptSaveJournal, err) }
	intent := SaveIntent{IntentID: wire.IntentID, Identity: identity, Snapshot: snapshot}
	if err := validateDecodedSaveIntent(wire.SchemaVersion, intent); err != nil { return 0, 0, SaveIntent{}, fmt.Errorf("%w: intent: %v", ErrCorruptSaveJournal, err) }
...
func saveJournalWireToSnapshot(schemaVersion uint16, wire saveJournalWireSnapshot) (Snapshot, error) {
	...
	if schemaVersion == LegacySaveJournalSchemaVersion { mp, maxMP = LegacyDefaultMaxMP, LegacyDefaultMaxMP }
	inventoryState := InventoryState{}; if schemaVersion >= InventorySaveJournalSchemaVersion { inventoryState = wire.Inventory }
	combatLoadout := skillloadout.Slots{}
	if schemaVersion >= LoadoutSaveJournalSchemaVersion { var err error; combatLoadout, err = combatLoadoutFromWire(wire.CombatLoadout); if err != nil { return Snapshot{}, err } }
	learnedSkills := learnedskills.Set{}
	if schemaVersion >= LearnedSkillsSaveJournalSchemaVersion { var err error; learnedSkills, err = learnedSkillsFromWire(wire.LearnedSkills); if err != nil { return Snapshot{}, err } }
	primaryStats := characterstats.Primary{}
	if wire.PrimaryStats != nil { primaryStats = characterstats.Primary{Strength: wire.PrimaryStats.Strength, Agility: wire.PrimaryStats.Agility} }
	snapshot := Snapshot{
...
func validateNewSaveIntent(intent SaveIntent) error {
	...
	return validateSnapshotV11(intent.Snapshot)
}
...
func validateDecodedSaveIntent(schemaVersion uint16, intent SaveIntent) error {
	...
	if schemaVersion < PrimaryStatsSaveJournalSchemaVersion && intent.Snapshot.PrimaryStats != (characterstats.Primary{}) { return ErrInvalidSnapshot }
	...
	case PrimaryStatsSaveJournalSchemaVersion:
		if !intent.Snapshot.Inventory.Initialized { return ErrInvalidSnapshot }
		if err := validateSnapshotV11(intent.Snapshot); err != nil { return err }
	default:
...
func makeSaveJournalFrame(payload []byte) []byte {
	frame := make([]byte, 4+len(payload)+4); binary.BigEndian.PutUint32(frame[:4], uint32(len(payload))); copy(frame[4:], payload); binary.BigEndian.PutUint32(frame[4+len(payload):], crc32.Checksum(payload, saveCRCTable)); return frame
}
