...
func (Codec) Unmarshal(messageType protocol.MessageType, data []byte) (protocol.Message, error) {
	...
	case protocol.MessageClientInitialClassSelection:
		var in clientInitialClassSelection
		if err := decodeStrict(data, &in); err != nil { return nil, err }
		return protocol.ClientInitialClassSelection{ClassID: in.ClassID}, nil
	...
	case protocol.MessageInitialClassSelectionResult:
		...
		if err := decodeStrict(data, &in); err != nil { return nil, err }
		return protocol.InitialClassSelectionResult{ClientActionSequence: in.ClientActionSequence, ClassID: in.ClassID, Outcome: protocol.InitialClassSelectionOutcome(in.Outcome), Reason: protocol.InitialClassSelectionRejectionReason(in.Reason)}, nil
	...
	case protocol.MessageWorldDynamicState:
		...
		for i, b := range in.Blockers { blockers[i] = protocol.WorldBlockerState{ID: b.ID, Enabled: b.Enabled} }
		gates := make([]protocol.WorldGateState, len(in.Gates))
		for i, g := range in.Gates { gates[i] = protocol.WorldGateState{ID: g.ID, HP: g.HP, MaxHP: g.MaxHP, Destroyed: g.Destroyed} }
	...
	case protocol.MessageEntityVitalsState:
		...
		if err := decodeStrict(data, &in); err != nil { return nil, err }
		return protocol.EntityVitalsState{EntityID: world.EntityID(in.EntityID), HP: in.HP, MaxHP: in.MaxHP, MP: in.MP, MaxMP: in.MaxMP, Defeated: in.Defeated, ReviveProtectionUntilTick: in.ReviveProtectionUntilTick}, nil
	case protocol.MessageInventorySnapshot:
		var in inventorySnapshot
		if err := decodeStrict(data, &in); err != nil { return nil, err }
	...
	case protocol.MessageShopSnapshot:
		var in shopSnapshot
		if err := decodeStrict(data, &in); err != nil { return nil, err }
		offers := make([]protocol.ShopOffer, len(in.Offers))
		for i, offer := range in.Offers { offers[i] = protocol.ShopOffer{OfferID: offer.OfferID, ItemArchetypeID: offer.ItemArchetypeID, Quantity: offer.Quantity, CostArchetypeID: offer.CostArchetypeID, CostQuantity: offer.CostQuantity} }
		return protocol.ShopSnapshot{Revision: in.Revision, NPCEntityID: world.EntityID(in.NPCEntityID), ShopID: in.ShopID, Offers: offers}, nil
