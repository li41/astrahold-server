...
func (Codec) Unmarshal(messageType protocol.MessageType, data []byte) (protocol.Message, error) {
	...
	case protocol.MessageShopSnapshot:
		...
		for i, offer := range in.Offers {
			offers[i] = protocol.ShopOffer{OfferID: offer.OfferID, ItemArchetypeID: offer.ItemArchetypeID, Quantity: offer.Quantity, CostArchetypeID: offer.CostArchetypeID, CostQuantity: offer.CostQuantity}
		}
