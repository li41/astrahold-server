...
type clientUseAction struct {
	...
	TargetZ    *float32 `json:"target_z,omitempty"`
}
...
type entityVitalsState struct {
	...
	ReviveProtectionUntilTick uint64 `json:"revive_protection_until_tick,omitempty"`
}
...
type equipmentSnapshot struct {
	Revision uint64               `json:"revision"`
	Slots    []equipmentSlotState `json:"slots"`
}
...
func toEntitySpawn(s protocol.EntitySpawn) entitySpawn {
	return entitySpawn{EntityID: uint64(s.EntityID), Kind: uint8(s.Kind), Transform: toEntityTransform(s.Transform), ArchetypeID: s.ArchetypeID}
}
