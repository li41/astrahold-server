...
func TestDefaultCatalogLocksLowTierEquipmentV1(t *testing.T) {
	...
	if got := catalog.Revision(); got != "low-tier-equipment-v1" { t.Fatalf("revision = %q", got) }
	...
	for _, tc := range cases {
		...
		if !ok { t.Fatalf("missing %s", tc.id) }
		if item.Kind != KindWeapon || item.Slot != SlotMainHand || item.Weapon == nil { t.Fatalf("%s not main-hand weapon: %#v", tc.id, item) }
	...
	for _, tc := range shields {
		item, ok := catalog.Resolve(tc.id)
		if !ok { t.Fatalf("missing %s", tc.id) }
		if item.Kind != KindShield || item.Slot != SlotOffHand || item.Shield == nil { t.Fatalf("%s not off-hand shield: %#v", tc.id, item) }
...
func TestCatalogRejectsMalformedDefinitions(t *testing.T) {
	...
	for name, def := range map[string]CatalogDefinition{
		...
		t.Run(name, func(t *testing.T) {
			if _, err := New(def); !errors.Is(err, ErrInvalidCatalog) { t.Fatalf("err = %v", err) }
		})
	}
}
