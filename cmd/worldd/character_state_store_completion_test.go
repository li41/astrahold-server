...
func TestPersistCharacterStateOutboxPublishesCompletionAfterStoreAndCheckpoint(t *testing.T) {
	dir := t.TempDir()
	...
	if processed != 1 || outbox.Depth() != 0 || checkpoint.RecordID != 1 {
		t.Fatalf("processed=%d pending=%d checkpoint=%#v", processed, outbox.Depth(), checkpoint)
	}
	loaded, exists, err := store.Load(identity)
	if err != nil || !exists || loaded.Snapshot.ClassID != classid.Oathguard {
		t.Fatalf("loaded=%#v exists=%v err=%v", loaded, exists, err)
	}
	if outbox.CompletionDepth() != 1 {
		t.Fatalf("completion depth=%d", outbox.CompletionDepth())
	}
	completed := outbox.Completed(1)
	if len(completed) != 1 || completed[0] != intent {
		t.Fatalf("completed=%#v intent=%#v", completed, intent)
	}
