...
func TestPersistCharacterStateOutboxCompletionStaysProcessLocal(t *testing.T) {
	dir := t.TempDir()
	...
	identity := worlddTrustedIdentity(t, "character:completion")
	...
	if err != nil || !exists || loaded.Snapshot != snapshot { t.Fatalf("loaded=%#v exists=%v err=%v", loaded, exists, err) }
