...
func main() {
	var (
		...
		churnTimeout       = flag.Duration("churn-timeout", 15*time.Second, "Maximum time from each teleport-churn trigger to lifecycle/reliable convergence")
		churnStable        = flag.Duration("churn-stable", 250*time.Millisecond, "How long each post-churn convergence condition must remain continuously true")
		churnRounds        = flag.Int("churn-rounds", 1, "Number of alternating swap/restore teleport-churn rounds")
	...
	if *clients <= 0 || *duration <= 0 || *readyTimeout <= 0 || *convergenceTimeout <= 0 || *churnTimeout <= 0 || *convergenceStable < 0 || *churnStable < 0 || *churnRounds <= 0 || *shutdownGrace < 0 {
		log.Fatal("clients/duration/ready-timeout/convergence-timeout/churn-timeout/churn-rounds must be > 0; convergence-stable/churn-stable/shutdown-grace must be >= 0")
	}
	if *allocProfileRate <= 0 {
		log.Fatal("alloc-profile-rate must be > 0")
	}
	profiler, err := newAllocProfiler(*allocProfilePrefix, *allocProfileRate)
	if err != nil {
		log.Fatalf("configure allocation profiler: %v", err)
	}
	if profiler != nil {
	...
	playerFactory, err := loadlab.NewPlayerFactory(loadedWorld.Definition, scenario, *clients)
	if err != nil {
		log.Fatal(err)
	}

	var churnSwapRequests []worldruntime.TeleportRequest
	var churnRestoreRequests []worldruntime.TeleportRequest
	if scenario == loadlab.ScenarioTeleportChurn {
		swapTargets, targetErr := loadlab.TeleportChurnTargets(loadedWorld.Definition, *clients)
		if targetErr != nil {
			log.Fatal(targetErr)
		}
		restoreTargets, restoreErr := loadlab.TeleportChurnRestoreTargets(loadedWorld.Definition, *clients)
		if restoreErr != nil {
			log.Fatal(restoreErr)
		}
		churnSwapRequests = make([]worldruntime.TeleportRequest, 0, len(swapTargets))
		for entityID, position := range swapTargets {
			churnSwapRequests = append(churnSwapRequests, worldruntime.TeleportRequest{EntityID: entityID, Position: position})
		}
		churnRestoreRequests = make([]worldruntime.TeleportRequest, 0, len(restoreTargets))
		for entityID, position := range restoreTargets {
			churnRestoreRequests = append(churnRestoreRequests, worldruntime.TeleportRequest{EntityID: entityID, Position: position})
		}
		sort.Slice(churnSwapRequests, func(i, j int) bool { return churnSwapRequests[i].EntityID < churnSwapRequests[j].EntityID })
		sort.Slice(churnRestoreRequests, func(i, j int) bool { return churnRestoreRequests[i].EntityID < churnRestoreRequests[j].EntityID })
	}
	...
	networkConfig.PlayerFactory = playerFactory
	...
	if err := loadlab.WriteReport(convergencePath, convergenceReport); err != nil {
		stop()
		<-serveDone
		<-loopDone
		log.Fatalf("write convergence report: %v", err)
	}
	if err := writeSlowTickReport(slowTickReportPath(convergencePath), convergenceSlowReport); err != nil {
		stop()
		<-serveDone
		<-loopDone
		log.Fatalf("write convergence slow tick report: %v", err)
	}
	...
	// Phase 2（teleport-churn only）：S3-E.8 可做 N 輪 swap/restore。
	// 每輪都必須先看到 non-converged，再等 Spawn/Despawn/Vitals + Reliable drain 重新成立。
	if scenario == loadlab.ScenarioTeleportChurn {
		latestMeta, err = runTeleportChurnRounds(ctx, *churnRounds, churnSwapRequests, churnRestoreRequests, worldRuntime, collector, slowTicks, convergence, server, scenario, *clients, *churnTimeout, *churnStable, *reportPath)
		if err != nil {
			stop()
			<-serveDone
			<-loopDone
	...
	if err := profiler.Write("before"); err != nil {
		stop()
		<-serveDone
		<-loopDone
		log.Fatalf("write allocation profile before measurement: %v", err)
	}
	collector.Reset()
	slowTicks.Reset()
	server.ResetNetworkMetrics()
	...
	if err := profiler.Write("after"); err != nil {
		stop()
		<-serveDone
		<-loopDone
		log.Fatalf("write allocation profile after measurement: %v", err)
	}
	...
	if err := writeSlowTickReport(slowTickReportPath(*reportPath), slowReport); err != nil {
		stop()
		<-serveDone
		<-loopDone
		log.Fatalf("write slow tick report: %v", err)
	}
